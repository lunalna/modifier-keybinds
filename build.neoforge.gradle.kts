...
tasks.register<PublishModrinthVersion>("modrinth") {
    ...
    versionNumber.set("${project.property("mod.version")}+${sc.current.version}-neoforge")
    versionName.set("${project.property("mod.name")} ${project.property("mod.version")} for ${sc.current.version} (NeoForge)")
    versionType.set("release")
    primaryFile.set(tasks.jar.flatMap { it.archiveFile })
...
tasks {
    processResources {
        ...
        exclude("pack.mcmeta", "META-INF/mods.toml", "fabric.mod.json", "*.ct", "*.classtweaker")
    }
